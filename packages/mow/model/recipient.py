...
class Table(object):
    def config_db(self, pkg):
        tbl = pkg.table('recipient', pkey='id', name_long='!!Recipient', name_plural='!!Recipients',caption_field='name_full')
        self.sysFields(tbl,counter='assigned_to')
        ...
        tbl.column('assigned_to',size='22' ,group='_',name_long='!!Assigned to').relation('staff.id',relation_name='recipients',mode='foreignkey',onDelete='raise')

        tbl.pyColumn('template_recipient',dtype='A',group='_',py_method='templateColumn', template_name='recipientcell')

    def trigger_onInserted(self,record):
        self.db.table('mow.recipient_calendar').alignCalendar(recipient_id=record['id'])